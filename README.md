...
### 복잡도!
- Complexity : 얼마나 증가하는지를 나타내는 수학적 개념
...
|--:|:--|--:|:--|
|1|$O(\log n)$|10억|단일 for문 반복문시 중간에 연산을 빠지는 경우가 매우 많은 경우(예 : 369)|
|2|$O(n)$|2천만|단일 for문 사용 / 반복문 하나 사용|
|3|$O(n \log n)$|1백만|2중 for문에서 반복문 하나가 log n 횟수로 연산|
|4|$O(n^2)$|5천|2중 for문|
|5|$O(n^3)$|3백|3중 for문|
|6|$O(2^n)$|25|재귀호출|
